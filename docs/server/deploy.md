...
> **2026-09-19 4단계(`claim-legacy`) 실행 — M6 통과** [확인됨·실물]: 1단계(백업, `db-20260919-1520.sql.gz`) → 3단계 사전 확인(`null_renders=0, null_formats=24, users=1`) 순서대로 진행. **4단계 진입 전 막힘**: `.env`의 `HARU_ADMIN_EMAIL`로 가입한 계정이 하나도 없었다(`users` 테이블 유일한 행 `suhday@naver.com`의 `role`이 `user`) — `AuthController.java:126`이 admin 역할을 가입 시점 이메일 일치로만 부여하기 때문이다. 사용자 결정으로 **이 기존 계정을 관리자로 승격**(`UPDATE users SET role='admin' WHERE email='suhday@naver.com'`, 단순 역할 변경, 가입 정보·비밀번호는 그대로). 승격 후 로그인·`POST /api/admin/claim-legacy` 실행 결과: `{"updatedFormatCount":24,"updatedScheduleCount":6,"updatedAssetCount":11,"updatedRenderCount":0,"updatedCommandCount":0,"updatedResultCount":0}`. 5단계 검증(`null_renders=0`, `null_formats=0`, 교차 소유 예약 0행) 전부 통과, Pi 로그에 `no owner_user_id`류 WARN 없음.
>
> **부작용 발견·해소**: claim-legacy 직후 약 1분간 Pi가 렌더 다운로드 404를 반복했다 — claim-legacy로 넘어간 레거시 포맷 24건 중 3건("Test Format", 2026-09-14~15 개발 테스트 데이터)에 **켜진 예약이 6개** 남아 있었고, 그 포맷들의 "최근 렌더"가 이미 삭제된 다른 테스트 계정(`110c9350-...`, claim-legacy 대상이 아닌 NOT-NULL 값이라 그대로 남아 있었다) 소유였던 탓에 `assertOwnership`이 불일치로 거부했다. 서버가 곧바로 새 렌더를 만들어 20:41:41에 자동 해소됐고(Pi 로그로 확인), 그 뒤 사용자 결정으로 이 6개 예약과 3개 "Test Format" 포맷을 SQL로 직접 삭제했다(`DELETE FROM schedules ...` → `DELETE FROM formats ...`). 최종 상태: `total_formats=22`, `enabled_schedules=0`(현재 실사용 예약 없음), Pi 정상 폴링. **교훈**: claim-legacy 5단계 검증(3개 쿼리)은 "포맷·스케줄 소유권"만 보고 "그 포맷의 최근 렌더가 실제 존재하는 다른(레거시) 소유자를 가리키는가"는 보지 않는다 — 이번 경우처럼 render.owner_user_id가 NULL이 아니라 존재하지만-옛날 소유자인 경우는 이 문서의 검증 쿼리로 걸러지지 않는다(자동 재렌더로 우연히 해소됐을 뿐).
