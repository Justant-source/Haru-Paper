...
- **그리기**: 인라인 SVG. 머리에 티커(굵게)·회사명(넓은 크기만)·마지막 값(크게, 굵게)+통화·전 거래일 대비(▲/▼/− + 값 + %)·마지막 봉 날짜(`M/D`, 거래소 현지 날짜) + **"종가" 또는 "현재가"**(아래 참고). 캔들스틱: **오른 날 = 속 빈 상자(흰 채움+검은 테두리), 내린 날 = 검게 채움**, 심지 선. 가로 점선 눈금 2~3개 + 가격 라벨(최고/최저 포함), 아래에 첫날·마지막 날 `M/D`.
...
- **"종가" vs "현재가" 표시** [확인됨, 2026-09-18 수정] — SOXL로 실사용 중 발견: 미국 정규장이 열려 있는 동안(마지막 봉이 그날 거래소 현지 날짜와 같고, 아직 정규장 마감 전) 조회하면 그 값은 계속 바뀌는 **실시간가**인데, 이걸 "종가"라고 표시하면 나중에(특히 다음날 아침 KST) 보는 사람에게는 마치 "전날 종가가 잘못 나온 것"처럼 보인다. `StockSeries.lastCandleSettled`(`YahooChartStockQuoteProvider`가 `meta.currentTradingPeriod.regular.end`와 날짜 비교로 판정)가 false면 라벨을 `M/D 현재가`로 바꾸고 조회 시각(KST, `HH:mm`)을 같이 보여준다. true면 기존처럼 `M/D 종가`.
- **실패 표시**: `errorBox(ticker, "시세를 가져오지 못했습니다")`, 없는 티커면 `"티커를 찾을 수 없습니다"`.
- **알려진 한계** [기본값]: `2x4`에 60봉을 넣으면 봉이 뭉개진다(51mm 해상도 한계). 예약 인쇄는 서버가 렌더를 미리 만들어 두고(오프라인 인쇄를 위해, `docs/architecture.md`) 인쇄 순간에 다시 렌더하지 않는다 — occurrence 60분 전에 한 번 더 렌더하지만, 그 뒤 최대 60분은 그 시점 값 그대로다. 07:00 KST 예약이면 재렌더가 보통 06:00경(겨울 EST 기준 미국 장 마감 시각과 근접)이라 "현재가"로 찍혀 인쇄될 가능성이 특히 높다 — 문서에 적고 수용한다.
