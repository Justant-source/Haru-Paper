...
| CG1 | 인터넷 없는 재부팅에서 시계 동기화 게이트(`ClockGate`)가 실제로 예약·명령·`last_tick_at`·순환 삭제를 보류시키는가 | Orange Pi, Wi-Fi 차단 후 재부팅 | **부분 판정, 2026-09-19 [확인됨·실물, 부정적]** — 의도한 "`ClockGate`만 앱 레벨에서 게이트"는 확인 못 했지만, 그 전제였던 "`time-sync.target`이 안 닿아도 에이전트 프로세스는 뜬다"가 **실물에서 거짓임을 확인했다**: 재부팅 후 `systemd-time-wait-sync.service`가 chrony는 정상 동기화됐는데도 "activating"에서 무한정 멈췄고, `haru-paper-agent.service`의 `After=...time-sync.target` 때문에 시작 job 자체가 `waiting`에 갇혀 **프로세스가 아예 안 떴다**(`ClockGate`는 호출되지도 못함). 원인 발견 즉시 `time-sync.target`을 `After=`에서 제거([policy.md](policy.md) 4절, `pi/deploy/haru-paper-agent.service`). **이 픽스 이후의 "진짜 인터넷 없는 재부팅에서 ClockGate가 혼자 정상 작동하는가"는 여전히 [미검증]** — `test_clock_gate.py` 21개 단위 테스트로만 확인된 상태는 그대로다 | PoC 완료 기준("WAN을 뽑은 상태에서도 07:00 인쇄") 시험 전에 반드시 재확인해야 한다 — 이번 사고로 시계 동기화 관련 systemd 설정이 바뀌었으므로, 바뀐 설정으로 다시 한번 "Wi-Fi 차단 후 재부팅 → ClockGate가 실제로 보류시키는가"를 봐야 한다 |
