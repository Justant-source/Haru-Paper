...
- **`pi/deploy/haru-paper-agent.service`와 `time-sync.target`을 2026-09-19부터 분리했다**: 원래는 `After=network-online.target time-sync.target`을 걸어 두고 "정상 부팅에서는 거의 즉시 통과, 인터넷 없는 재부팅에서만 `ClockGate`가 앱 레벨로 막는다"는 게 전제였다(CG1, [hardware-verification.md](hardware-verification.md) 참고, 실물 미검증 상태였음). 실제로는 `systemd-time-wait-sync.service`가 chrony는 정상 동기화됐는데도 "activating"에서 무한정 멈추는 사고가 실물에서 발생했고([policy.md](policy.md) 4절), 그러면 `time-sync.target`이 영영 도달하지 않아 `After=`로 걸린 에이전트의 시작 job이 `waiting`에 갇혀 **`ClockGate`가 개입할 기회도 없이 프로세스 자체가 안 떴다**. 그래서 `time-sync.target`을 `After=`에서 뺐다 — 이제 에이전트는 항상 뜨고, 미동기 판단은 전적으로 `ClockGate`(위 항목들)가 맡는다.
