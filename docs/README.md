...
| M5 | Pi 실물 설치, 연결 방식 결정 | **완료(4/4).** 재부팅 자동시작·서버 폴링·transport=`bt` 확정에 이어, `pi/transport/bt.py` 구현(콜드 ACL 워크어라운드 포함) + Pi ↔ M832 페어링 + 실물 인쇄 1회(텍스트+그레이데이션+체커보드, 사용자 육안 확인) 전부 [확인됨·실물, 2026-09-17]. `HARU_PRINTER_DRIVER=m832` 상시. **2026-09-19: 앱 "지금 인쇄" → 서버 → 에이전트 실행기 → 프린터 → 서버 결과 업로드 전체 경로를 처음으로 실물 확인**[확인됨·실물, pi/agent.md 11절] — 종이 출력 육안 확인, 서버 `status=printed` 일치 |
