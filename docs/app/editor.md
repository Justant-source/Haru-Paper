...
| 2 | **문제해결을 다이얼로그로 먼저, 상세는 별도 화면** — 용지 부족 시 모달로 먼저 뜨고 "해결 방법 보기"를 눌러야 4단계 전체화면 안내 | `lack_paper_solution_dialog.xml`→`_activity.xml` | `manual_flag` 정책에서 용지 미장착으로 인쇄가 막혔을 때, 지금처럼 토글 안내만 보여주지 말고 "왜 막혔는지 + 어떻게 해결"을 다이얼로그→상세 2단계로 분리 |
