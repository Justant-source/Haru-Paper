...
- **프론트엔드 UX 참고 자료**: `~/Data/Phomemo-App`(저장소 밖, 비공개 로컬 폴더) — Phomemo 공식
  앱을 구조·흐름 파악 목적으로 디컴파일 분석한 레이아웃·네비게이션·UX 패턴. 화면·편집기를 새로
  설계할 때 참고하되, 원본 이미지·코드는 옮기지 않는다. 편집기 관련 패턴 발췌는 [editor.md](editor.md) 11절
