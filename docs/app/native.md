...
PoC 범위를 넘어가면 앱 기술보다 먼저 아래가 바뀐다. **회원가입·로그인, 다수 사용자, 기기를 계정에 등록, 기기 토큰은 이미 M6에서 구현됐다**(웹앱 기준) — 남은 것은 네이티브 앱으로의 이식뿐이고, 새로 설계할 항목은 아니다. `docs/app/web.md`·`screens.md`·[../server/auth.md](../server/auth.md) 참고.

| 항목 | PoC 현재 | 상용화 시 검토 |
|---|---|---|
| 접속 경로 | Tailscale 내부망(웹앱) | 공개 접속 경로(M10, 예: Cloudflare Tunnel). 인증 자체는 이미 세션 기반(M6)이라 별도 설계 불필요 |
| 알림 | 없음(이력 화면에서 확인) | 푸시 알림(인쇄 실패·용지 없음·Pi 오프라인) — FCM/APNs |
| 동기화 | Pi 30초 폴링 | 실시간 WebSocket(지금 인쇄 즉시 반영) |
| Pi 초기 설정 | SD카드에 Wi-Fi·SSH 키를 미리 넣는 헤드리스 설치 | 앱에서 Pi 페어링·Wi-Fi 설정 전달(BLE 프로비저닝 등) — 계정 등록·기기 토큰 자체는 이미 있음 |
| 포맷 공유 | JSON 파일 가져오기/내보내기 | 공유 갤러리(M9 마켓) |
