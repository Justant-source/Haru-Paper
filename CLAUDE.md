...
- 2단계 기기(ESP32 인라인 어댑터)는 `.temp/02-esp32-디바이스-계획서-v1.3.md`. Pi PoC가 끝나기 전에는 부품을 사지 않는다
...
| M5 Pi 실물 | 4개 중 3개 완료. 남은 것: Pi에서 M832 페어링 → `pi/transport/bt.py` 작성 → `.env` 전환 → 실물 인쇄 1회 |
...
- ESP32 부품 구매는 `.temp/02` 게이트를 통과한 뒤에만. 데이터 경로(USB 호스트) 사용자 확인 전에는 사지 않는다
...
| `docs/pi/hardware-verification.md` | V0~V4·H4·H5 상태표, 결정 규칙, findings 링크 |
...
| `.temp/01`·`02`·`03`·`04` | 진행 중 계획: Orange Pi PoC v1.4 / ESP32 v1.3 / 플랫폼 M6~M10 / 드래그 편집기. 완료된 항목은 docs로, 끝나면 삭제 |
