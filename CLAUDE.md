...
| M1 드라이버 · M4 에이전트 | 코드 있음, Pi에서 상시 구동 중이나 **배포본이 뒤처져 있다** — Pi는 `581899e`까지만 반영(12커밋 뒤), Pi 코드 커밋 3개(용지 게이트 fail-closed `623a8dc`, 재시도·명령 실행기·결과 업로드 `8b7194b`, 시계 게이트·되돌아보기·순환 삭제 `51a6a8d`) 전부 미배포라 [미검증]. **Pi는 옛 fail-open 용지 정책 코드로 구동 중 — 배포 전에 `HARU_PAPER_POLICY`를 `unverified`에서 바꾸지 않는다**(절대금지 1). M1의 "실물 1회"는 M5로 이월 |
