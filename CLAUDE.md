...
- 프론트엔드 UX 참고 자료: `~/Data/Phomemo-App`(저장소 밖, 비공개 로컬 폴더) — Phomemo 공식 앱을
  구조·흐름 파악 목적으로 디컴파일 분석한 산출물(레이아웃·네비게이션·UX 패턴). 원본 이미지·코드는
  옮기지 않으며, 이 저장소로 가져올 때는 `docs/app/editor.md` 11절처럼 패턴 설명만 인용한다
...
## 현재 상태 (2026-09-18 — 마일스톤 통과 시 이 절과 `docs/README.md`를 같이 고친다)

| 항목 | 상태 |
|---|---|
| M0 뼈대 · M2 서버 · M3 웹앱 | 완료 |
| M1 드라이버 · M4 에이전트 | 코드 있음, Pi에서 상시 구동 중. **2026-09-18에 배포 따라잡음** [확인됨·실물] — Pi가 `581899e`(12커밋 뒤)에서 `74a5384`까지 `git pull --ff-only`로 갱신, `systemctl restart`. 용지 게이트 fail-closed(`623a8dc`), 재시도·명령 실행기·결과 업로드(`8b7194b`), 시계 게이트·되돌아보기·순환 삭제(`51a6a8d`), SSE 깨우기 채널이 전부 반영됐다. 재기동 로그로 poll·scheduler tick·SSE 연결 정상 확인(`docs/pi/setup.md` 5.4절). `HARU_PAPER_POLICY=unverified` 유지(절대금지 1). M1의 "실물 1회"는 M5로 이월. **2026-09-18 사용자 결정으로 `manual_flag`로 전환**[확인됨·실물] — 앱의 "용지 장착됨" 토글이 켜져 있을 때만 예약 인쇄가 실제로 나간다. 같은 날 Pi의 DNS 문제(NetworkManager가 Tailscale MagicDNS를 반영 못 함, `docs/environment.md`)도 발견·수정 |
| M5 Pi 실물 | **완료(4/4).** Pi ↔ M832 페어링, `pi/transport/bt.py`(콜드 ACL 워크어라운드 포함), 실물 인쇄 1회 육안 확인(드라이버·전송 계층 직접 호출). `HARU_PRINTER_DRIVER=m832` 상시. **2026-09-19: 앱 "지금 인쇄"→서버→에이전트 실행기→프린터→서버 결과 업로드 전체 경로를 처음 실물 확인**[확인됨·실물] — 종이 출력 육안 확인, `status=printed` 일치(`docs/pi/agent.md` 11절) |
| M6 계정·기기 | 구현 완료(세션 로그인, 기기별 토큰, 페어링 코드, 관리자). **통과.** V4 백필(2026-09-18)에 이어 **2026-09-19 `claim-legacy` 실행 완료**[확인됨·실물, `docs/server/deploy.md` 7.1절] — 사전 백업 후 관리자 계정 승격(가입된 admin 계정이 없어 기존 계정을 승격) → 실행(`updatedFormatCount=24`) → 검증(`null_renders=0`, `null_formats=0`, 교차 소유 예약 0행) 전부 통과. 실행 직후 레거시 테스트 포맷 3건("Test Format")의 켜진 예약 6개가 옛 소유자를 가리키던 렌더 때문에 약 1분간 404를 냈다가 자동 재렌더로 해소됐고, 사용자 결정으로 그 테스트 예약·포맷은 삭제했다(상세는 deploy.md). `HARU_OWNERSHIP_STRICT`는 아직 `false`(7.2절은 실물 인쇄 확인이 선행 조건, 이번 범위 밖) |
| M7 레이아웃·위젯 | 부분 — 2026-09-18: 포맷 스키마를 **v3(위젯 그리드)**로 교체(v2 행/슬롯·드래그 편집기 폐기), 위젯 6종(`dateHeader`/`text`/`image`/`morningLetter`/`stockChart`/`weather`) 구현. **같은 날 운영 스택에 재배포**(`docker compose build && up -d`, 사전 수동 백업)[확인됨·실물] — Flyway는 마이그레이션 없이 스키마 V4 유지, `e2e-smoke.sh` 65 PASS/0 FAIL(관리자 절 포함, `claim-legacy` 테스트 후 원상 복구까지 확인: `null_formats=24` 그대로). **실물 인쇄(종이에 찍힌 결과를 눈으로 확인)는 2026-09-19에 확인됨**[확인됨·실물, 아래 항목·`docs/pi/agent.md` 11절] — 단 이때 인쇄한 "아침 브리핑" 포맷은 `weather`·`stockChart`·`morningLetter` 3종이고 `image` 위젯은 포함되지 않았다. 사용자 스크립트 러너 `haru-widget-runner`는 **미구현**. **2026-09-19: Phomemo 앱 UX 대조로 찾은 필수 3종 보강**[확인됨·실물, 3커밋 + 운영 배포] — ① 이미지 위젯 편집 활성화(`AssetField.tsx`, `catalog=true`) + 위젯 asset 처리를 `WidgetRegistry`로 이관해 `"image"` 하드코딩 경계 위반 제거 + 에셋 소유권 검증(IDOR 수정) ② 홈·예약에 인쇄 막힘 배너(`PrintBlockedBanner`, `paper_off`가 폴링 지연보다 항상 우선) ③ 이력에 그날 실제로 나간 렌더 썸네일(`GET /api/history/{resultId}/render.png`, 지금 다시 렌더하는 `preview.png`와 다름). 서버 테스트 217개·앱 build/lint 통과 후 **같은 날 운영 스택에 재배포**(사전 백업, `docker compose build && up -d`, `docs/server/deploy.md` 5절) — Flyway는 마이그레이션 없이 V4 유지, `e2e-smoke.sh` **69 PASS/0 FAIL**, 별도 테스트 계정으로 `image.catalog=true` 직접 확인(사용 후 삭제). **같은 날 실물 인쇄로 "지금 인쇄" 전체 경로는 확인됐지만, 이 3종 보강 자체(이미지 위젯 업로드·인쇄 막힘 배너·이력 렌더 이미지)는 이번 인쇄에 관여하지 않아 그 기능들의 실물 인쇄 확인은 아직 [미검증]**으로 남아 있다 |
| M8~M10 | 미착수 |
